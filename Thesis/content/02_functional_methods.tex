\chapter{Functional Methods in Quantum Field Theory}\label{chap:FuncMethods}
...
\end{figure}
\end{subequations}
 where $\partial_{t} R_{k, ij}(p, q)=\partial_{t} R_{k}(p^2)(2 \pi)^{d} \  \delta_{i j} \ \delta_{\mathrm{D}}(p-q)$ and therefore the trace in on the r.\,h.\,s. effectively sums over just one index $i$ and integrates over one loop momentum $p$.
