...
\begin{align}
	\mathcal{M}_{\mu\nu} &= \bar{\nabla}^{\rho}(g_{\mu\nu}\nabla_{\rho} + g_{\rho\nu}\nabla_{\mu}) - \bar{\nabla}_{\mu}\nabla_{\nu}.
\end{align}
If we choose the gauge parameters such that $\alpha = \beta \equiv 0$, we are able to reduce the dynamical degrees of freedom to two, namely the transverse-traceless mode $h^{\mathrm{TT}}$ and the trace mode $h^{\mathrm{Tr}}$. This choice allows us to simplify the full graviton propagator a lot. We are left with:
\begin{equation} G_{h h}=\left[\frac{1}{\Gamma^{(2)}+R_{k}}\right]_{h h} = \frac{32\pi}{Z_h}
\begin{pmatrix}
\frac{1}{\bar{\Delta}\left[1+r_{k}\right]+-2\Lambda\frac{2}{3}\mathcal{R}} & 0 & 0 & 0 \\[10pt]
0 & 0  & 0 & 0 \\[10pt]
0 & 0 & \frac{-\frac{8}{3}}{\bar{\Delta}\left[1+r_{k}\right]-\frac{4}{3} \Lambda}  &0\\[10pt]
0 & 0 & 0 & 0 \\
...
\begin{equation}
	\Gamma^{(2)}_k+R_{k}=\Gamma^{(2)}_k\left(\Delta \rightarrow \Delta\left[1+r_{k}(x)\right]\right),
\end{equation}
where the shape function $r_k$ is again of Litim-type (\ref{eqn:Litim}). \\
%TODO: Results for second mode + gauge fixing contributions (?) 
...
\begin{equation}
\begin{aligned}
	\frac{1}{2}\tr{\left(\Gamma^{(2)}_{k, \phi\phi}\right)^{-1}\partial_t R_{k,S}} &= \frac{1}{2}\tr{\frac{Z_S\cdot\bar{\Delta}\left(\partial_t r_k - \eta_s r_k\right)}{Z_S \cdot\bar{\Delta}\left(1 + r_k\right)}\mathbbm{1}_S}\\
...
\begin{equation}
	\frac{1}{\left(i\slashed{\nabla}\right)} = \frac{1}{\left(i\slashed{\nabla}\right)}\times\frac{\left(i\slashed{\nabla}\right)}{\left(i\slashed{\nabla}\right)} = \frac{\left(i\slashed{\nabla}\right)}{\Delta_{\left(\sfrac{1}{2}\right)}}
\end{equation}
and therefore the full fermion propagator reads
\begin{equation}
	\left(\Gamma^{(2)}_{k, \bar{\psi}\psi} \right)^{-1} = \frac{\left(i\bar{\slashed{\nabla}}\right)}{Z_D\cdot\bar{\Delta}_{\left(\sfrac{1}{2}\right)}\sqrt{1+r_{k,S}}}.
\end{equation}
